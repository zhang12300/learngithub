...
#define GryoFlatFlagNum 15    //陀螺仪，平路判断计数限值。连续计数超过此值，则判断为平路。
...
//HJZ
#define GyroResultNum 10    //陀螺仪采样结果作差时，间隔的点数。
#define GyroResultLimit 13  //陀螺仪采样结果作差时，要求差的大小。
//AD采样结果。
...
extern uint32 GryoFlatCount; //陀螺仪，平路计数。
extern uint8 GryoFlatFlag;   //陀螺仪，平路标志。1平路，0坡。
extern int16 GyroResult[GyroResultNum];  //陀螺仪，角度储存数组。
extern uint8 GyroResultFlag;   //陀螺仪，坡道判断。1坡，0平路。

extern uint8 GyroInit(void);
extern uint8 GryoDeal(void);

extern uint8 GyroResultForbidFlag;           //坡道禁止赛道类型判断标志，1表示禁止任何赛道类型判断，0表示不禁止。
extern uint8 GyroResultForbidBackCount;      //坡道禁止赛道类型判断倒计数
extern uint8 GyroResultForbidBackCountNum;  //坡道禁止赛道类型判断倒计数载入值
