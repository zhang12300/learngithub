...
/* Private macro -------------------------------------------------------------*/
//SD卡CS片选使能端操作：
#define SD_CS_ENABLE()      (gpio_ctrl(PTA_BASE_PTR,13,0))//选中SD卡
#define SD_CS_DISABLE()     (gpio_ctrl(PTA_BASE_PTR,13,1))//不选中SD卡
...
unsigned char SD_ReadSingleBlock(unsigned long sector, unsigned char *buffer);             
															//读一个sector
unsigned char SD_WriteSingleBlock(unsigned long sector,const unsigned char *buffer);       
															//写一个sector
