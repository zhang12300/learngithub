...
#define __VECTORS_H     1
    ...
    extern uint32 QuadLeftCount;       //左编码器计数值，越大表示速度越快。
    extern uint32 QuadRightCount;      //右编码器计数值，越大表示速度越快。
    extern volatile uint8 QuadDoneFlag;         //编码器速度值采集完成标志，0未完成，1完成。
    
    extern uint16 DiffCountL;        //用于差速测量时，左轮脉冲数的总计数。
    extern uint16 DiffCountR;        //用于差速测量时，右轮脉冲数的总计数。

    extern uint8 PIT2_Isr_Change;    //PIT2的功能复用选择。0表示用于发车时的5秒计数，1表示用于定时停车。
    extern uint8 LightHouseWaitStartTooLongFlag;   //灯塔等久标志位置位
